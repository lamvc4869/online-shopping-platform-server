...
const updateUserService = async (userId, updateData, updateFile) => {
  try {
    ...
    const allowedFields = ['firstName', 'lastName', 'email', 'password', 'role', 'age', 'address', 'phoneNumber'];
    ...
    for (const [key, value] of Object.entries(updateData)) {
      if (allowedFields.includes(key)) {
        if (key === 'password') {
          ...
          const existingEmail = await User.findOne({ email: value, _id: { $ne: userId } });
          if (existingEmail) {
            return "Email đã được sử dụng bởi user khác";
          }
          user[key] = value;
        } else {
          user[key] = value;
