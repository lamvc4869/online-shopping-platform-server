...
const ProductDetails = () => {
  ...
  useEffect(() => {
    if (product) {
      ...
      const productsCopy = (Array.isArray(products) ? products : []).filter(
  ...
  useEffect(() => {
    setThumbnail(
      product?.image && product.image.length ? product.image[0] : null
    );
  }, [product]);
  ...
  const hasMoreProducts =
    relatedProducts.filter((p) => p.inStock).length > displayCount;
          ...
          <div className="text-center mb-8">
            ...
            {relatedProducts.filter((p) => p.inStock).length > 0 && (
              <p className="text-gray-600 mt-3">
                ...
                {relatedProducts.filter((p) => p.inStock).length} products
              </p>
            )}
          </div>
